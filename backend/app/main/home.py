...
def get_user(user_id):
    ...
    if g.current_user.get("is_admin"):
        row["is_banned"] = bool(row["banned_at"])
...
@main.post("/api/users/<int:user_id>/ban")
@admin_required
def ban_user(user_id):
    data = request.get_json(silent=True) or {}
    reason = data.get("reason")
    if reason is not None and reason not in MODERATION_REASONS:
        return error("Некорректная причина блокировки")
    if user_id == g.current_user["id"]:
        return error("Нельзя заблокировать собственный аккаунт")

    with transaction() as cursor:
        cursor.execute(_query("SELECT id,is_admin,banned_at FROM email_auth WHERE id=%s"), (user_id,))
        target = cursor.fetchone()
        if not target:
            return error("Пользователь не найден", 404, "not_found")
        target = dict(target)
        if target["is_admin"]:
            return error("Нельзя заблокировать аккаунт администратора", 403, "protected_account")
        if target["banned_at"]:
            return error("Аккаунт уже заблокирован", 409, "already_banned")
        cursor.execute(
            _query("UPDATE email_auth SET banned_at=CURRENT_TIMESTAMP,banned_by=%s,ban_reason=%s,updated_at=CURRENT_TIMESTAMP WHERE id=%s"),
            (g.current_user["id"], reason, user_id),
        )
        cursor.execute(_query("UPDATE sessions SET revoked_at=CURRENT_TIMESTAMP WHERE user_id=%s AND revoked_at IS NULL"), (user_id,))
        cursor.execute(
            _query("INSERT INTO user_ban_audit(moderator_user_id,user_id,action,reason) VALUES(%s,%s,%s,%s)"),
            (g.current_user["id"], user_id, "ban", reason),
        )
    return jsonify({"success": True, "user_id": user_id, "is_banned": True})


@main.delete("/api/users/<int:user_id>/ban")
@admin_required
def unban_user(user_id):
    with transaction() as cursor:
        ...
        target = dict(target)
        if target["is_admin"]:
            return error("Нельзя изменить блокировку аккаунта администратора", 403, "protected_account")
        if not target["banned_at"]:
            return error("Аккаунт не заблокирован", 409, "not_banned")
        cursor.execute(
            _query("UPDATE email_auth SET banned_at=NULL,banned_by=NULL,ban_reason=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=%s"),
            (user_id,),
        )
        cursor.execute(
            _query("INSERT INTO user_ban_audit(moderator_user_id,user_id,action) VALUES(%s,%s,%s)"),
