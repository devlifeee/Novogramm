...
def register():
    ...
    try:
        ...
        current_app.logger.error(f"Ошибка БД: {str(e)}")
    ...
    # Хэшируем пароль для временного хранения (будет сохранен после подтверждения)
    hashed_password = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
    
    # Сохраняем временные данные пользователя
    try:
        execute_query(
            "INSERT INTO temp_users (email, password_hash) VALUES (%s, %s) ON CONFLICT (email) DO UPDATE SET password_hash = %s",
            (email, hashed_password.decode('utf-8'), hashed_password.decode('utf-8'))
        )
    except Exception as e:
        current_app.logger.error(f"Ошибка сохранения временных данных: {str(e)}")
        return jsonify({'success': False, 'error': 'Ошибка при сохранении временных данных'}), 500
...
def confirmation_page():
    ...
    return render_template('auth/confirmation.html', email=email)
...
def verify_code():
    try:
        ...
        if result:
            ...
            delete_query = "DELETE FROM verification_codes WHERE email = %s AND code = %s"
            execute_query(delete_query, (email, code))
            
            # Получаем хэш пароля из временной таблицы
            temp_user_query = "SELECT password_hash FROM temp_users WHERE email = %s"
            temp_user = execute_query(temp_user_query, (email,), fetch=True)
            
            if not temp_user:
                return jsonify({
                    'success': False, 
                    'error': 'Данные пользователя не найдены. Пожалуйста, зарегистрируйтесь снова.'
                }), 400
            
            # Сохраняем пользователя в основную таблицу
            insert_query = """
                INSERT INTO email_auth (email, password, verified) 
                VALUES (%s, %s, TRUE)
                RETURNING id
            """
            user_result = execute_query(insert_query, (email, temp_user['password_hash']), fetch=True)
            
            # Удаляем временные данные
            execute_query("DELETE FROM temp_users WHERE email = %s", (email,))
...
def complete_registration():
    ...
    # Валидация username
    if not re.match(r'^[a-z0-9_.]+$', username):$', username):
