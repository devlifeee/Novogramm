...
def test_admin_can_assign_and_remove_clown_hat(client, users):
    target, admin = users
    promote_to_admin(admin)
    post = client.post("/api/posts", json={"content": "hat post"}, headers=headers(target)).get_json()["post"]
    comment = client.post(
        f"/api/posts/{post['id']}/comments",
        json={"content": "hat comment"},
        headers=headers(target),
    ).get_json()

    assert client.post(f"/api/users/{target['id']}/clown-hat").status_code == 401
    assert client.post(f"/api/users/{target['id']}/clown-hat", headers=headers(target)).status_code == 403

    response = client.post(f"/api/users/{target['id']}/clown-hat", headers=headers(admin))
    assert response.status_code == 200
    assert response.get_json() == {"success": True, "user_id": target["id"], "has_clown_hat": True}

    profile = client.get(f"/api/get_user/{target['id']}", headers=headers(admin)).get_json()["user"]
    assert profile["has_clown_hat"] is True
    feed_post = client.get(f"/api/posts/{post['id']}", headers=headers(admin)).get_json()["post"]
    assert feed_post["user_has_clown_hat"] is True
    comments = client.get(f"/api/posts/{post['id']}/comments", headers=headers(admin)).get_json()
    assert comments[0]["id"] == comment["id"]
    assert comments[0]["user_has_clown_hat"] is True

    audit = execute_query(
        "SELECT moderator_user_id,user_id,action,created_at FROM user_clown_hat_audit WHERE user_id=%s ORDER BY id DESC LIMIT 1",
        (target["id"],),
        fetch=True,
    )
    assert audit["moderator_user_id"] == admin["id"]
    ...
    response = client.delete(f"/api/users/{target['id']}/clown-hat", headers=headers(admin))
    assert response.status_code == 200
    assert response.get_json() == {"success": True, "user_id": target["id"], "has_clown_hat": False}
